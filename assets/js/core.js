...
const CURRENCY_OPTIONS=[
  {code:'RUB',symbol:'₽',name:'الروبل',shortName:'روبل'},
  {code:'EGP',symbol:'E£',name:'الجنيه المصري',shortName:'جنيه'},
  {code:'SAR',symbol:'ر.س',name:'الريال السعودي',shortName:'ريال سعودي'},
  {code:'USD',symbol:'$',name:'الدولار الأمريكي',shortName:'دولار أمريكي'},
];
...
function getCurrencyOption(code){
  ...
  return CURRENCY_OPTIONS.find(option=>option.code===safeCode)||CURRENCY_OPTIONS[0];
}
function currencyCode(){return getCurrencyOption(S&&S.settings&&S.settings.currency).code;}
function currencySymbol(){return getCurrencyOption(currencyCode()).symbol;}
function currencyName(){return getCurrencyOption(currencyCode()).name;}
function currencyShortName(){return getCurrencyOption(currencyCode()).shortName;}
function formatMoneyAmount(value,options={}){
  const prefix=String(options.prefix||'');
  return `${prefix}${toArFull(value)} ${currencySymbol()}`.trim();
}
function renderCurrencyOptionsMarkup(selectedCode=currencyCode()){
  return CURRENCY_OPTIONS.map(option=>`<option value="${option.code}" ${option.code===selectedCode?'selected':''}>${option.name}</option>`).join('');
}
...
const ACHIEVEMENT_DEFS=[
  ...
  {id:'saved_1000',icon:'🏦',title:'١٠٠٠ محفوظ',desc:'اجمعي ١٠٠٠ ادخار'},
...
function getChallengeText(id){
  if(id==='save_200')return `وفري ${toAr(200)} ${currencyShortName()} هذا الأسبوع`;
  const def=WEEKLY_CHALLENGE_DEFS.find(item=>item.id===id);
  return def?def.text:'';
}
function getAchievementCopy(id){
  if(id==='saved_1000'){
    return {
      title:`${toAr(1000)} ${currencyShortName()} محفوظ`,
      desc:`اجمعي ${toAr(1000)} ${currencyShortName()} ادخار`,
    };
  }
  const item=ACHIEVEMENT_DEFS.find(value=>value.id===id);
...
const SUPABASE_URL='https://gneqvzfggpurvnfblewo.supabase.co';
const SUPABASE_ANON_KEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImduZXF2emZnZ3B1cnZuZmJsZXdvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM2MTkwNTksImV4cCI6MjA4OTE5NTA1OX0.YiazO-4j0llYMzEgrJhNw4vKl0L0kN0-ohNOzTqkeao';
const SAMA_STATE_ID='sama_user_1';
...
function createDefaultState(){
  return {
    ...
    weeklyHistory:[],
    moodLog:[],
    settings:{fontScale:1,language:'ar',currency:'RUB'},
